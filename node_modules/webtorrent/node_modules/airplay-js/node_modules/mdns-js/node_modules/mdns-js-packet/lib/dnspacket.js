...
DNSPacket.parse = function (buffer) {
  ...
  SECTION_NAMES.forEach(function (sectionName) {
    ...
    for (var i = 0; i < section.length; i++) {
      ...
      debug('parsed type `%d` for section %s', part.type, sectionName);
      if (part.type === DNSRecord.Type.OPT) {
        if (receivedOpts++ >= 0) {
          //TODO: does it only ever be in additonal.
          if (sectionName === 'additional') {
            packet.edns_version = part.opt.version;
            packet.do = part.opt.do;
            packet.payload = part.class;
          }
        }
        else {
